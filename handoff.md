...
The standard renderer path combines a narrow aqua horizon, stronger turquoise lower field, two slow low-frequency wave terms on high quality (one on balanced/low tiers), analytic wave gradients for surface normals, broad and tight white/cyan sunlight reflections, moving ripple ribbons, restrained lime environmental bounce, and a lightweight cursor-driven water highlight. Water opacity is intentionally strong enough to remain visible behind the existing CSS ambience and glass. The renderer uses a capped device-pixel ratio and bounded animation cadence; GPU bubbles, caustics, the hero orb, and refraction remain dormant so navigation does not recompile the background. The original authored CSS bubbles remain visible; clouds and all DOM content remain intact.

The refraction branch remains isolated and dormant; it does not capture DOM pixels and remains reject/defer for production. Aero Lab and the song player remain removed and must not be restored or referenced.
